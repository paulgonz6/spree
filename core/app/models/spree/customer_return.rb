module Spree
  class CustomerReturn < Spree::Base
    ...
    def fully_reimbursed?
      !return_items.undecided.exists? && return_items.decided.all? { |return_item| return_item.reimbursement.try(:reimbursed?) }
    end

    def reimbursements_allowed?
      !return_items.undecided.exists?
    end

    def reimbursements_required?
      return_items.where(acceptance_status: 'accepted', reimbursement_id: nil).exists?
    end

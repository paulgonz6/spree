  ...
  module Stock
    class Coordinator
      ...
      def build_location_configured_packages(packages = Array.new)
        order.order_stock_locations.where(shipment_fulfilled: false).each do |stock_location_configuration|
          units = unallocated_inventory_units.select{|iu| iu.variant == stock_location_configuration.variant}.take(stock_location_configuration.quantity)
          packer = build_packer(stock_location_configuration.stock_location, units)
      ...
      def build_packages(packages)
        StockLocation.active do |stock_location|

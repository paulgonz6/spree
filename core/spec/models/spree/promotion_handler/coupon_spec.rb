  ...
  module PromotionHandler
    describe Coupon do
      ...
      def expect_adjustment_creation(adjustable:, promotion:, promotion_code:nil)
        expect(adjustable.adjustments.flat_map(&:source).map(&:promotion)).to include(promotion)
        expect(adjustable.adjustments.last.promotion_code).to eq promotion_code
      end
      ...
      context "existing coupon code promotion" do
        ...
        context "with a whole-order adjustment action" do
          ...
          context "right coupon given" do
            ...
            it "promotion code hit max usage" do
              promotion_code.update!(usage_limit: 1)
              coupon = Coupon.new(order)
              coupon.apply
              expect(coupon.successful?).to be true
